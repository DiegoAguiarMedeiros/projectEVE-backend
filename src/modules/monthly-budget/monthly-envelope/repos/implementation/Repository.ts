...
export class Repository implements Interface {
    ...
    async getAll(id: string, page?: number, pageSize?: number, orderBy?: string, order?: string): Promise<MonthlyEnvelope[]> {
    ...
    async getById(id: string, userId: string): Promise<MonthlyEnvelope | null> {
    ...
    async create(data: MonthlyEnvelope): Promise<void> {
        const rawData = await Mapper.toPersistence(data);
